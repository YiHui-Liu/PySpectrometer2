...
while cap.isOpened():
    ...
    if ret == True:
        frame = cv2.resize(frame0, (frameWidth, frameHeight))
        y0 = int((frameHeight / 2) - 40)  # origin of the vertical crop
        # y=200 	#origin of the vert crop
        x0 = 0  # origin of the horiz crop
        h0 = 80  # height of the crop
        w0 = frameWidth  # width of the crop
        cropped0 = frame[y0 : y0 + h0, x0 : x0 + w0]
        ...
        h = 20
        w = 150
        cv2.line(cropped0, (x, y), (x + w, y), (255, 255, 255), 1)
        cv2.line(cropped0, (x, y + h), (x + w, y + h), (255, 255, 255), 1)
        cropped = cropped0[y : y + h, x : x + w]
        cropped = cv2.resize(cropped, (w0, h0))
        ...
        logi = logi - np.min(logi)
        imax = 0.9 * 320 / np.max(logi)
        for i in logi:
            rgb = wavelength_to_rgb(
                round(wavelengthData[index])
            ...
            cv2.line(
                graph,
                (index, 320),
                (index, 320 - int(np.floor(imax * i))),
                (b, g, r),
                1,
            )
            cv2.line(
                graph,
                (index, 319 - int(np.floor(imax * i))),
                (index, 320 - int(np.floor(imax * i))),
                (0, 0, 0),
                1,
                cv2.LINE_AA,
        ...
        # print(indexes)
        for i in indexes:
            height = 310 - int(np.floor(imax * logi[i]))
        ...
        if measure == True:
            ...
            cv2.line(
                graph,
                (cursorX, cursorY - 140 - h0),
                (cursorX, cursorY - 180 - h0),
                (0, 0, 0),
                1,
            )
            cv2.line(
                graph,
                (cursorX - 20, cursorY - 160 - h0),
                (cursorX + 20, cursorY - 160 - h0),
                (0, 0, 0),
                1,
            )
        ...
        if clickArray:
            for data in clickArray:
                mouseX = data[0]
                mouseY = data[1]
                cv2.circle(graph, (mouseX, mouseY - h0), 5, (0, 0, 0), -1)
        ...
        spectrum_vertical = np.vstack((messages, cropped0, cropped, graph))
        # dividing lines...
